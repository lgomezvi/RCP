...
2.  **Analyze and Breakdown:** Decompose the Query into a logical, sequential flow of high-level steps using **only** the actions defined in the documentation. If the query is a simple, direct action that can be translated into an action, ask for any specific details if needed, and provide a concise, single sentence of the plan. 
